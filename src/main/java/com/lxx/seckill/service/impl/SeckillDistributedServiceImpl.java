...
import com.lxx.seckill.distributedlock.zookeeper.ZKLockUtil;
...
public class SeckillDistributedServiceImpl implements ISeckillDistributedService {
    ...
    public Result startSeckillRedisLock(long seckillId, long userId) {
        ...
        boolean res = false;
        try {
            res = RedisLockUtil.tryLock(seckillId+"", TimeUnit.SECONDS, 3, 20);
            if(res){
                ...
                return Result.error(SeckillStatusEnum.MUCH);
            }
        } catch (Exception e) {
